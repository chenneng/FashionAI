...
parser.add_argument('--attr_epochs', type = int, default = 35, help = 'number of each attr epochs')
...
parser.add_argument('--save_dir', type = str, default = './weights', help = 'log and model save dir')
...
for i in range(8):
    ...
    for epoch in range(start_epoch, args.attr_epochs):
        if args.adjust_lr:
            adjust_lr(optimizer,  i * args.attr_epochs + epoch)
        ...
        if epoch % args.save_interval == 0 or epoch == args.attr_epochs - 1:
            torch.save(model.state_dict(),  ('weights/model_{}.pth'.format(i * args.attr_epochs + epoch)))
